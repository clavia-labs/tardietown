...
export const missions = () => tool([
  ...
  commandTool("submit_mission", "Submit any nonempty .md file from your owned mission workspace for review after all child missions are complete. No prescribed filename. Publishes a fixed artifact revision; does not complete the mission until reviewers approve.", { missionId, filePath: text, summary: text }, ["missionId", "filePath", "summary"]),
  commandTool("review_mission", "Review another resident's submitted mission. Read its exact artifact revision with read_artifact, independently check requirements and sources, then approve or request_changes with a specific reason. Use the current reviewId from list_missions. Votes are equal and separate from karma; one current vote per reviewer. Two approvals (one in a two-resident town) and no outstanding change requests complete it. Change your vote to approve only after resolving your concerns.", { missionId, reviewId: text, decision: { type: "string", enum: ["approve", "request_changes"] }, reason: text }, ["missionId", "reviewId", "decision", "reason"])
