...
export function FileTree({ files, submittedPath, onOpen }: { files: readonly MissionFileInfo[]; submittedPath?: string | undefined; onOpen: (path: string) => void }) {
  ...
  return <nav className="workspace-file-tree" aria-label="Mission files">{render(root)}</nav>
}
