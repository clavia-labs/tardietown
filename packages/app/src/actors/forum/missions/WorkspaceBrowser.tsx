...
import { ArrowLeft, X, ChevronRight, FolderOpen, Folder } from "lucide-react"
...
function MissionFolder({ mission, missions, reader, onFile, onPublished }: FolderProps) {
  ...
  return <li><Disclosure open={open} onOpenChange={setOpen}>
    <DisclosureSummary title={mission.description}><ChevronRight className="file-tree-chevron" size={13} aria-hidden="true" /><Folder size={15} aria-hidden="true" /><span>{mission.description}</span><span className="workspace-mission-flairs"><span className="forum-mission-kind">{mission.parentMissionId ? "Child mission" : "Town mission"}</span><span className={`forum-mission-status forum-mission-status-${mission.status}`}>{mission.status.replaceAll("_", " ")}</span></span></DisclosureSummary>
...
export function WorkspaceBrowser({ missions, residents, reader, artifacts, readArtifact, onClose, initialPath, initialRevision }: {
  ...
  return <>
    ...
    <aside data-scene-card="workspace" className="artifact-browser workspace-browser mission-workspace" aria-label="Workspace" style={published ? { display: "none" } : undefined}>
      <header><h2><FolderOpen size={16} aria-hidden="true" /> Workspace</h2><IconButton variant="ghost" className="panel-icon-button" type="button" onClick={onClose} label="Close workspace"><X size={18} strokeWidth={1.75} aria-hidden="true" /></IconButton></header>
      {selected && <nav><IconButton variant="ghost" className="panel-icon-button" type="button" label="Back to files" onClick={() => setSelected(undefined)}><ArrowLeft size={18} aria-hidden="true" /></IconButton><span title={`${selected.mission.description}/${selected.path}`}>{selected.path}</span></nav>}
