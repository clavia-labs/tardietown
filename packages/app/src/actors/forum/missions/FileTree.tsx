import { Action } from "../../../town/ui/controls"
...
export function FileTree({ files, submittedPath, onOpen, published }: { published?: { path: string; revision: number | undefined; onOpen: () => void } | undefined; files: readonly MissionFileInfo[]; submittedPath?: string | undefined; onOpen: (path: string) => void }) {
  ...
  const render = (directory: Directory) => <ul>
    {[...directory.folders].sort(([a], [b]) => a.localeCompare(b)).map(([name, child]) => <li key={`folder:${name}`}>
      <Disclosure defaultOpen><DisclosureSummary><ChevronRight className="file-tree-chevron" size={13} aria-hidden="true" /><Folder size={15} strokeWidth={1.6} aria-hidden="true" /><span>{name}</span></DisclosureSummary>{render(child)}</Disclosure>
    </li>)}
    {[...directory.files].sort((a, b) => a.path.localeCompare(b.path)).map(file => <li key={file.path}>
      <Action type="button" title={file.path} onClick={() => onOpen(file.path)}><FileText size={15} strokeWidth={1.6} aria-hidden="true" /><span>{file.path.split("/").at(-1)}</span>{file.path === submittedPath && <small className="file-tree-submitted">Submitted</small>}<small>v{file.revision}</small></Action>
    </li>)}
  </ul>
  return <nav className="workspace-file-tree" aria-label="Mission files">
    {published && <ul><li><Action type="button" title={published.path} onClick={published.onOpen}><FileText size={15} strokeWidth={1.6} aria-hidden="true" /><span>{published.path.split("/").at(-1)}</span><small className="file-tree-submitted">Published</small><small>v{published.revision}</small></Action></li></ul>}
    {render(root)}
  </nav>
}
