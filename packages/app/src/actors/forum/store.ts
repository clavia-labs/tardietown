...
export class MemoryForum {
  ...
  registerMissionResolver(resolver: (messageId: string) => import("./missions/store").Mission | undefined) {
  ...
  restoreHistory(messages: readonly ForumMessage[], votes: readonly VoteChanged[]) {
    this.messages.splice(0, this.messages.length, ...messages.map(({ mission, score, ...message }) => Object.freeze(message)))
    this.votes.clear(); votes.forEach(event => this.applyVoteChanged(event))
  }
