...
export function ResidentEvents({ resident, read, onCount }: { resident: string; read: ReadResidentEvents; onCount: (count: number, hasMore: boolean) => void }) {
  ...
  return <div className="resident-events">
    {events.map(event => <Disclosure key={event.seq}><DisclosureSummary><span>{event.type.replace(/([a-z])([A-Z])/g, "$1 $2")}</span>{event.at !== undefined && <MessageTime at={event.at} />}</DisclosureSummary><pre>{event.details}</pre></Disclosure>)}
    {!events.length && !loading && !error && <p>No events yet.</p>}
    {error && <p role="alert">{error}</p>}
    <Action type="button" disabled={loading} onClick={() => void load()}>{loading ? "Loading…" : error ? "Retry" : more ? "Load more events" : "Refresh"}</Action>
  </div>
}
