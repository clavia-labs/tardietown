...
export class ArtifactStore {
  ...
  restoreHistory(documents: readonly ArtifactDocument[]) {
    for (const file of documents) {
      const versions = this.files.get(file.path) ?? []
      if (!versions.some(version => version.revision === file.revision)) versions.push(Object.freeze(file))
      this.files.set(file.path, versions.sort((a, b) => a.revision - b.revision))
    }
  }
